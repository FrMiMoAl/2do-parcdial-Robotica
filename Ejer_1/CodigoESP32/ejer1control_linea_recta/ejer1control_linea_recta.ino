...
void cmd_vel_callback(const void * msgin) {
  ...
  float base_rpm = linear * 100.0;   
  base_rpm = constrain(base_rpm, -100.0, 100.0);
...
void control_loop_pid() {
  ...
  if (target_rpm_a > 0.1) {
    ...
    if (abs(pwm_output_a) < PWM_DEADBAND) {
      pwm_output_a = 0;
      pid_motor_a.reset();
    }
  ...
  if (target_rpm_b > 0.1) {
    ...
    if (abs(pwm_output_b) < PWM_DEADBAND) {
      pwm_output_b = 0;
      pid_motor_b.reset();
    }
...
void update_yaw_improved() {
  ...
  float filtered_gyro_x = gyro_filter_alpha * gyro_x + 
                          (1.0 - gyro_filter_alpha) * _prev_gyro_x_filtered;
...
void recalibrate_gyro_offset_dynamic() {
  if (abs(target_rpm_a) < 5.0 && abs(target_rpm_b) < 5.0) {
    ...
    gyro_x_offset = gyro_recal_old_weight * gyro_x_offset + 
                    gyro_recal_new_weight * new_offset;
  }
}
...
void setup() {
  ...
  Serial.println("\n=== ROBOT ESP32 - VERSIÓN CORREGIDA ===");
  Serial.print("Filter Alpha: ");
  Serial.println(velocity_filter_alpha);
  Serial.print("Kp Motor: 0.6 (aumentado de 0.4)");
  Serial.println();
